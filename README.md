...
Czech is my mother tongue, so the app generated by the as-is scripts is in Czech. Translating the UI to a language of your choice will be a trivial task for your AI agent.
...
4. Set the API key environment variable named by `api_key_env`, usually `OPENAI_API_KEY`.
...
For quick UI experiments, start with `sample_data/video_descriptions.json`. The sample keeps the production schema and representative metadata, but its VLM text fields were translated from Czech to English for readability. The field names remain `headline_cs` and `description_cs` because that is what the current scripts expect.
